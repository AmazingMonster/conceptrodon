...
`Legation` accepts a stockroom and returns the stockroom via member `Rail`.
...
```Haskell
Legation ::   template<template<auto...> class...> class...
...
constexpr auto fun(Legation<Sto_0>) { return false; }

constexpr auto fun(Legation<Sto_1>) { return true; }

static_assert(not fun(Legation<Sto_0>{}));

static_assert(fun(Legation<Sto_1>{}));
```

If we only want compilers to perform an argument-dependent lookup, we can utilize `decltype` and `std::declval` to avoid defining the function body.

```C++
constexpr auto gun(Legation<Sto_0>) -> std::false_type;

constexpr auto gun(Legation<Sto_1>) -> std::true_type;

static_assert(not decltype(gun(std::declval<Legation<Sto_0>>()))::value);

static_assert(decltype(gun(std::declval<Legation<Sto_1>>()))::value);
```
...
- [source code](../../../conceptrodon/legation.hpp)
