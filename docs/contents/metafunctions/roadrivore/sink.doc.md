...
In contrast to `Skip` and `Skid,` `Sink` introduces a new scope to avoid name collision. Hence, it uses the name `Road` for both initiation and recursion.<pre><code>   Operation{FirstLayer{...LastLayer}}
