...
`Roadrivore::Reveal` accepts an operation and returns a metafunction such that when invoked, the metafunction becomes the member `type` of the previously provided operation invoked by the arguments.
