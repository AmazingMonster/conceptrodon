...
`Typelivore::TypicalPop` accepts a list of elements and pops the last one. The result is stored in `Capsule` and returned via alias member `type`.
...
**Base step:** Pop the last element from a fixed-length list.
